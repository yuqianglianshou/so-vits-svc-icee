...
- 检查训练前依赖与底模
- 导入缺失文件到标准目录
- 补齐 `model_assets/dependencies/encoders/`、`model_assets/dependencies/vocoders/` 和 `model_assets/dependencies/base_models/44k/` 下的训练依赖与底模

当前训练页支持：

- 导入 `training_data/source/<说话人目录>/*.wav`
- 自动建议目录名
- 删除当前说话人目录
- 自动同步处理后目录 `training_data/processed/44k/<说话人目录>`
...
- 本地目录：`model_assets/dependencies/encoders/contentvec_hf/`
- 目录内需要包含：
  - `config.json`
  - `model.safetensors`
- 当前锁定来源：
...
+ 预训练底模文件：
  ...
  + 放在`model_assets/dependencies/base_models/44k`目录下
...
+ 扩散模型预训练底模文件：
  ...
  + 放在`model_assets/dependencies/base_models/44k/diffusion`目录下

训练开始时，程序会自动把这组底模同步到当前模型工作区目录，所以 `model_assets/workspaces/<模型名>/` 主要表示训练产物目录，而不是长期存放底模的目录。
...
+ 预训练的 NSF-HIFIGAN 声码器 ：[nsf_hifigan_20221211.zip](https://github.com/openvpi/vocoders/releases/download/nsf-hifigan-v1/nsf_hifigan_20221211.zip)
  + 解压后，将四个文件放在`model_assets/dependencies/vocoders/nsf_hifigan`目录下
...
# 也可手动下载放在 model_assets/dependencies/vocoders/nsf_hifigan 目录
...
+ 下载模型[rmvpe.zip](https://github.com/yxlllc/RMVPE/releases/download/230917/rmvpe.zip)，目前首推该权重。
  + 解压缩`rmvpe.zip`，并将其中的`model.pt`文件改名为`rmvpe.pt`并放在`model_assets/dependencies/encoders`目录下
...
执行完以上步骤后，`training_data/processed` 目录便是预处理完成的数据。当前主模型训练会依赖第 3 步生成的 `*.train.pt` 统一特征包；如果这些文件缺失，训练会提示你重新执行第 3 步。
...
+ 训练过程：
  ...
  + 执行`python cluster/train_cluster.py`，模型的输出会在`model_assets/workspaces/<模型名>/kmeans_10000.pt`
  + 聚类模型目前可以使用 gpu 进行训练，执行`python cluster/train_cluster.py --gpu`
+ 推理过程：
  + `services/inference_main.py`中指定`cluster_model_path` 为模型输出文件，留空则默认为`model_assets/workspaces/<模型名>/kmeans_10000.pt`
...
+ 推理过程：
  ...
  + `services/inference_main.py`中指定`cluster_model_path` 为模型输出文件，留空则默认为`model_assets/workspaces/<模型名>/feature_and_index.pkl`
