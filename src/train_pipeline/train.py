...
from src.data_utils import TextAudioCollate, TextAudioSpeakerLoader
from src.models import (
    MultiPeriodDiscriminator,
    SynthesizerTrn,
)
from src.modules.losses import discriminator_loss, feature_loss, generator_loss, kl_loss
from src.modules.mel_processing import mel_spectrogram_torch, spec_to_mel_torch
from src.path_utils import get_sovits_d0_path, get_sovits_g0_path
...
def run(rank, n_gpus, hps, use_ddp=True):
    ...
    pin_memory = hps.train.get("pin_memory")
    if pin_memory is None:
        pin_memory = True
    persistent_workers = hps.train.get("persistent_workers")
    ...
    if rank == 0:
        ...
        eval_num_workers = hps.train.get("eval_num_workers")
        if eval_num_workers is None:
            eval_num_workers = 1
        eval_pin_memory = hps.train.get("eval_pin_memory")
        if eval_pin_memory is None:
            eval_pin_memory = False
        eval_persistent_workers = hps.train.get("eval_persistent_workers")
    ...
    try:
        ...
        base_g = get_sovits_g0_path()
        base_d = get_sovits_d0_path()
        if base_g.exists() and base_d.exists():
            utils.load_checkpoint(str(base_g), net_g, optim_g, skip_optimizer=True)
            utils.load_checkpoint(str(base_d), net_d, optim_d, skip_optimizer=True)
        epoch_str = 1
        global_step = 0
    if skip_optimizer:
        epoch_str = 1
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                ...
                if keep_ckpts > 0:
                    preserve_g_steps = getattr(hps.train, "keep_g_milestones", None)
                    utils.clean_checkpoints(
                        path_to_models=hps.model_dir,
                        n_ckpts_to_keep=keep_ckpts,
                        sort_by_time=True,
                        preserve_g_steps=preserve_g_steps,
                    )
