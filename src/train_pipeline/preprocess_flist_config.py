...
from src.diffusion.logger import utils as du
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_dir", type=str, default="./model_assets/workspaces/44k/training_data/processed/44k", help="path to source dir")
    ...
    spk_dict = {}
    spk_id = 0
    source_dir_name = os.path.basename(os.path.abspath(args.source_dir))
    speakers = [source_dir_name]
    wav_files = [
        file_name
        for file_name in os.listdir(args.source_dir)
        if os.path.isfile(os.path.join(args.source_dir, file_name))
        and file_name.lower().endswith(".wav")
        and not file_name.startswith(".")
    ]
    if not wav_files:
        raise RuntimeError(
            f"单说话人模式要求 {args.source_dir} 直接包含 wav 文件；当前未检测到可用 wav。"
        )
    ...
    for speaker in tqdm(speakers):
        ...
        for file_name in wav_files:
            ...
            file_path = os.path.join(args.source_dir, file_name).replace("\\", "/")
    ...
    if args.tiny:
        config_template["train"]["epochs"] = min(int(config_template["train"].get("epochs", 10000)), 10000)
        config_template["train"]["keep_g_milestones"] = [1000, 2000, 5000, 10000, 20000, 50000, 100000]
