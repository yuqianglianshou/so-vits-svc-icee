...
from src.train_ui.text import compact_stage_line, status_tone_tag
from src.train_ui.workspace import count_raw_dataset_wavs, count_training_wavs, speaker_dirs_in_train_root
...
def collect_stage_state(model_name: str = "44k", raw_dir: str = "default_dataset", train_dir: str = "training_data/processed/44k"):
    ...
    if train_wavs == 0:
        ...
        detail = f"处理后数据目录应直接包含当前数据集的 wav 文件；当前结构不符合要求（统计结果：train_speakers={train_speakers}，目录标记：{'、'.join(train_speaker_dirs) or '无'}）"
        stage_lines.append(compact_stage_line("2. 生成配置与文件列表", "未满足前置条件", detail))
        stage_items.append(("2. 生成配置与文件列表", "未满足前置条件", detail))
        if next_step is None:
            next_step = "先清理处理后数据目录，确保当前目录直接包含本轮训练的 wav 文件。"
